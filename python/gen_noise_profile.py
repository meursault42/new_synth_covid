...
def decomp_window(input_vec,likely_index,start_value,adj_window):
    min_loss = 99999
    best_p = 0
    result_a = 0
    result_m = 0
    out_list = []
    weight = 0
    for i in likely_index:
        adj_vec = input_vec[(start_value-math.floor(i/2)):(start_value+adj_window+math.floor(i/2))]
        #input_vec[(start_value-math.floor(i/2)+1):(start_value+adj_window+(math.floor(i/2))+1)]            
        in_result = seasonal_decompose(adj_vec,model='multiplicative',period=i)
        in_result_2 = seasonal_decompose(adj_vec,model='additive',period=i)
        resid = in_result.resid
        resid = resid[~np.isnan(resid)]
        tot_resid = np.sum(abs(np.diff(np.diff(resid))))
        
        inner_weight = np.mean(abs(in_result.seasonal)/in_result.observed)
        ...
        if tot_resid < min_loss:
            ...
            best_p = i
            result_m = in_result
            result_a = in_result_2
            weight = inner_weight
    return min_loss, best_p, out_list, result_m, result_a, weight
...
def _nth_order_sort(seq,n_order,sort='low'):
    '''
    Parameters
    ----------
    seq : A Numpy array
    ...
    if sort=='low':
        for i in range(n_order):
                ...
                seq = seq[seq!=seq_min]
        return min(seq)
def emission_hmm(input_vec,start_window=30,start_value=5,prior_step=0,
                 prior_magnitude=0,n_order=0, prior_dir_dict=0, cutoff = 0.05,
                 prior_noise=0,likely_index=[5,6,7,8,9,11]):
    '''
    ...
    if len(adj_vec)>=start_window:
        '''
        b_l, b_val, stl_out, res, w= decomp_window(input_vec,likely_index=likely_index,
                                            start_value=start_value,
                                            adj_window=adj_window)
        #extract dirction of periodic component as %change from mean
        qpc = res.seasonal[b_val:b_val*2]/np.sum(res.seasonal[b_val:b_val*2])
        #add to dict
        key = str(b_val)+'_day'
        ...
        #extract weight
        magnitude_vec = np.append(magnitude_vec, w)
        
        #noise_list.append(r_ts_fit(adj_vec,13))
        '''
        b_l, b_val, stl_out, res_m, res_a, w= decomp_window(input_vec,likely_index=likely_index,
                                            start_value=start_value,
                                            adj_window=start_window)
        #extract dirction of periodic component as %change from mean
        #test code! remove!
        '''
        signal = np.sum(abs(res_a.trend[~np.isnan(res_a.trend)]))+np.sum(abs(res_a.seasonal[~np.isnan(res_a.seasonal)]))
        noise = np.sum(abs(res_a.resid[~np.isnan(res_a.resid)]))
        snr = signal/noise        
        
        un_pad = math.floor(b_val/2)
        seas = (res_a.seasonal[~np.isnan(res_a.seasonal)])[un_pad:len(res_a.seasonal[~np.isnan(res_a.seasonal)])-un_pad]
        trend = res_a.trend[~np.isnan(res_a.trend)]
        noise = res_a.resid[~np.isnan(res_a.resid)]
        
        #r2t_sd = np.std(noise/trend+seas)
        r2t_sd = np.std(noise/trend)
        s1 = np.mean(seas**2+trend**2)/np.mean(noise**2)
        '''
        
        ###double test code
        signal = np.sum(abs(res_m.trend[~np.isnan(res_m.trend)]))+np.sum(abs(res_m.seasonal[~np.isnan(res_m.seasonal)]))
        noise = np.sum(abs(res_m.resid[~np.isnan(res_m.resid)]))
        snr = signal/noise        
        
        un_pad = math.floor(b_val/2)
        seas = (res_m.seasonal[~np.isnan(res_m.seasonal)])[un_pad:len(res_m.seasonal[~np.isnan(res_m.seasonal)])-un_pad]
        trend = res_m.trend[~np.isnan(res_m.trend)]
        noise = res_m.resid[~np.isnan(res_m.resid)]
        ...
        #noise_list.append(r_ts_fit(adj_vec,13))
        noise_list.append(r2t_sd)
    
        return (emission_hmm(input_vec=input_vec,\
...
def get_hmm_outputs(subset_index,input_data_df,n_order=0,**kwargs):
    '''
    ...
    for ind in subset_index:
        ...
        emm, mag, dir_dict_loc, nn = emission_hmm(input_vec=data_vec_loc,n_order=n_order)#,start_window=window)
        emm=emm.astype(int)
        emm=emm.reshape(-1,1)
        a_list.append(emm)
        b_list.append(mag)
...
def _gen_hmm_noise_seq(input_ts,model,mag_vec,dir_dict,mag_weight=1):
    '''
    ...
    local_dir_dict = dict()
    hmm_chain = model[0]
    #temp testing code, remove later
    #mag_vec  = mag_vec * 1.5
    #magnitude_distribution = mag_vec * mag_weight
    #magnitude_distribution = mag_vec[mag_vec<1] * mag_weight
    magnitude_distribution = np.random.normal(.5,.1,1000)
    magnitude_distribution = magnitude_distribution[magnitude_distribution >.2]
    magnitude_distribution = magnitude_distribution[magnitude_distribution <.9]
...
def generate_noise_sequences(input_ts_list, subset_index, input_data_df,
                             ...
                             shuffle_prob = 30, smooth_transitions=False,**kwargs):
    '''
...
'''
def decomp_window_vals(seq,start,end,offset):
    min_loss = 99999
    best_p = 0
    out_list = []
    for i in likely_index:
        ...
        result = seasonal_decompose(
            seq[(starting_offset+start-padding):(starting_offset+end+(padding))],
        ...
        #plt.plot(resid)
        #np.sum(resid)
        #actual loss function is here, sum 2nd derivative, aka curvature
        tot_resid = np.sum(abs(np.diff(np.diff(resid))))
        #plt.plot(np.diff(np.diff(resid)))
        out_list.append(tot_resid)
        if tot_resid < min_loss:
            min_loss = tot_resid
            best_p = i
    return min_loss, best_p, out_list
'''
def _emission_hmm_stl(input_vec,start_window=14,start_value=0,prior_step=0,
                 prior_magnitude=0,n_order=0, prior_dir_dict=0, cutoff = 0.05,
                 prior_noise=0,likely_index=[5,7,9,11,13]):
    '''
    Function recursively iterates through a given array and returns the 
    nth order periodic component found, along with the magnitude and
    the averaged direction vector. 
    ...
    if isinstance(prior_noise, list):
        ...
        noise_list = []
    #adj_window = min(start_window,(len(input_vec[start_value:])))
    adj_vec = input_vec[start_value:(start_value+start_window)]
    
    if len(adj_vec)>=start_window:
        b_l, b_val, stl_out, res, w= decomp_window_stl(input_vec,likely_index=likely_index,
                                            start_value=start_value,
                                            adj_window=start_window)
        #extract dirction of periodic component as %change from mean
        qpc = res.seasonal[b_val:b_val*2]/np.sum(res.seasonal[b_val:b_val*2])
        ...
        return (_emission_hmm_stl(input_vec=input_vec,\
                      start_value=(b_val+start_value),\
                      prior_step=output_vec,\
                      prior_magnitude=magnitude_vec,\
                      n_order=n_order,\
...
def decomp_window_stl(input_vec,likely_index,start_value,adj_window):
    min_loss = 99999
    best_p = 0
    result = 0
    out_list = []
    ...
    for i in likely_index:
        ...
        adj_vec_t = np.tile(adj_vec,20)
        #input_vec[(start_value-math.floor(i/2)+1):(start_value+adj_window+(math.floor(i/2))+1)]            
        #turn into ts
        ts = pd.Series(np.log(adj_vec_t),
                       index=pd.date_range('1-1-2020', periods=len(adj_vec_t),freq='D'),
                       name = 'seq')

        stl = STL(ts, seasonal=i)
        res = stl.fit()
        res.plot()
        
        tot_resid = np.sum(abs(np.diff(np.diff(res.resid ))))
        inner_weight =  max(0,(1-np.var(res.resid)/np.var(res.seasonal + res.resid)  ))
        ...
        if tot_resid < min_loss:
            ...
            result = res
            weight = inner_weight
    return min_loss, best_p, out_list, result, weight